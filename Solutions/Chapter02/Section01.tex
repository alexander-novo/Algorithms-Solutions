...
\begin{document}
	...
	\begin{enumerate}[leftmargin=\labelsep]
		...
		% 2.1-3
		\item Consider the \textbf{\textit{searching problem}}:
		\begin{description}
		...
		\begin{answer}
			...
			\begin{description}
				...
				\item[Output:] An $(n + 1)$-element sequence $C = \langle c_1, c_2, \dots, c_{n + 1} \rangle$ which represents the sum of the two binary numbers given above, i.e.
				\[
			...
			\begin{align}
				1\cdot2^i + 1\cdot2^i &= 2\cdot2^i \nonumber\\
				                      &= 1\cdot2^{i + 1} + 0\cdot2^i. \label{eq:binary-add-carry}
			\end{align}
			...
			\begin{addmargin}[2em]{2em}
				...
				\[
					\sum_{j = 1}^{i - 1} c_j2^{j - 1} + (carry_{(i - 1)}) \cdot 2^{i-1} = \qty(\sum_{j = 1}^{i - 1} a_j 2^{j - 1}) + \qty(\sum_{j = 1}^{i - 1} b_j 2^{j - 1}).
				\]
			\end{addmargin}
			...
			\begin{description}
				...
				\item[Maintenance:] Observe that $carry \in \{0,1\}$ for the entire algorithm. Since $A[i], B[i], carry_{(i - 1)} \leq 1$, then $A[i] + B[i] + carry_{(i - 1)} \leq 3$. Then we have two cases:
				\begin{description}
					\item[Case 1:] $A[i] + B[i] + carry_{(i - 1)} \leq 1$. Then $C[i] \in \{0, 1\}$, the next $carry_i = 0$, and
					\begin{align*}
						\sum_{j = 1}^{i} c_j2^{j - 1} + (carry_i) \cdot 2^{i} &= \sum_{j = 1}^{i - 1} c_j2^{j - 1} + c_i2^{i - 1}\\
						                                                      &= \sum_{j = 1}^{i - 1} c_j2^{j - 1} + \qty(a_i + b_i + (carry_{(i - 1)}))\\
					\end{align*}
				\end{description}
			\end{description}
		\end{answer}
